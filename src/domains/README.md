...
テンプレートは `src/templates/` および `src/components/` からインポートできます:
...
// UIコンポーネント
import { ListPage, DetailPage, FormPage } from '@/components/pages';
import { Loading, Error, Empty } from '@/components/feedback';
...
詳細は `README.md`（リポジトリルート）を参照してください。
