...
export interface LoginPageProps {
  ...
  /** ログインAPIエンドポイント */
  apiEndpoint?: string;
  /** ログイン成功後のリダイレクト先（onSuccess 未指定時に使用） */
  redirectUrl?: string;
  ...
  /** ログイン成功コールバック（指定時はリダイレクトを呼び出し側に委譲） */
  onSuccess?: (data: { userId: string; role: string; name: string }) => void;
}
    ...
    <div className="flex min-h-screen items-center justify-center bg-base-50 px-4">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="rounded-lg border border-base-900/10 bg-white p-8">
          {error && (
            <div className="mb-4 rounded-full bg-red-50 px-4 py-2.5 text-sm text-red-600">
              {error}
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full rounded-full bg-base-900 px-6 py-3 text-base font-medium text-white transition-colors duration-200 hover:bg-base-900/85 disabled:cursor-not-allowed disabled:opacity-50"
          >
