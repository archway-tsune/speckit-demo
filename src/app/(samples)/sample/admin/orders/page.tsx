...
export default function AdminOrdersPage() {
  ...
  const { data, isLoading } = useFetch<{ orders: Order[]; pagination: { page: number; limit: number; total: number; totalPages: number } }>('/sample/api/orders', fetchParams, { loginUrl: '/sample/login' });
                ...
                <tbody>
                  {orders.map((order) => (
                    <tr key={order.id} data-testid="order-row" onClick={() => router.push(`/sample/admin/orders/${order.id}`)} className="cursor-pointer border-b border-base-900/10 last:border-0 hover:bg-base-50">
                      ...
                      <td className="px-4 py-3"><span data-testid="order-status" className="rounded-full bg-base-100 px-3 py-1 text-xs font-medium">{orderStatusLabels[order.status]}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
