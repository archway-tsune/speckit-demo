...
export default function AdminOrderDetailPage() {
  ...
  const { data: rawOrder, isLoading, refetch } = useFetch<Order>(`/sample/api/orders/${params.id}`, undefined, { loginUrl: '/sample/login' });
  ...
  return (
    <div>
      <BackButton label="注文一覧に戻る" onClick={() => router.push('/sample/admin/orders')} />
      ...
      >
        {(o) => (
          <>
            <AlertBanner variant={message.includes('失敗') ? 'error' : 'success'} message={message} />
            <div className="grid gap-6 lg:grid-cols-2">
              <div className="rounded-lg border border-base-900/10 bg-white p-6">
                ...
                <dl className="space-y-3 text-sm">
                  <div><dt className="text-base-900/60">注文ID</dt><dd className="mt-1 font-medium">{o.id}</dd></div>
                  <div><dt className="text-base-900/60">注文日時</dt><dd className="mt-1">{formatDateTime(o.createdAt)}</dd></div>
                  <div><dt className="text-base-900/60">更新日時</dt><dd className="mt-1">{formatDateTime(o.updatedAt)}</dd></div>
                </dl>
              </div>
              <div className="rounded-lg border border-base-900/10 bg-white p-6" data-testid="customer-info">
                ...
                <dl className="space-y-3 text-sm">
                  <div><dt className="text-base-900/60">ユーザーID</dt><dd className="mt-1 font-medium">{o.userId}</dd></div>
                </dl>
              </div>
            </div>
            ...
            <div className="mt-6 rounded-lg border border-base-900/10 bg-white p-6">
              ...
              <div className="flex items-center gap-4">
                <span data-testid="order-status" className="rounded-full bg-base-100 px-3 py-1 text-sm font-medium">{orderStatusLabels[o.status]}</span>
                ...
                <Button onClick={handleUpdateStatus} disabled={isUpdating || selectedStatus === o.status}>
