...
カート内数量が在庫数に達している場合はカート追加を拒否し、在庫不足のエラーメッセージを表示する。
...
在庫数を超える数量に変更しようとした場合は変更を拒否し、在庫不足のエラーメッセージをユーザーに通知する。
...
引数なし。TDD、スタブ置換、contracts 準拠、navLinks 有効化などの共通指示は constitution に定義済みのため、カート固有の設計判断がなければ追加入力は不要。
