...
注文作成時、カートの税抜小計を基に消費税（10%、端数切り捨て）と合計金額（税込）を算出し、注文エンティティに小計・消費税・合計金額の3つを記録する。
注文作成後、カートは自動的にクリアされる。
カートが空の場合は注文不可とし、「注文手続きへ」ボタンは非表示にする。

## 注文履歴・注文詳細
購入者は自分の注文のみ一覧で確認できる（注文ID先頭8桁・注文日・商品数・合計金額・ステータス、1ページあたり20件のページネーション付き。ページ送りで過去の注文を閲覧可能）。
注文詳細画面で注文ID・注文日時・ステータス・商品一覧・合計金額を確認できる。
他人の注文にはアクセスできない。

## 管理者の注文管理
管理者は全ユーザーの注文を一覧で確認でき、ステータスで絞り込める。一覧は1ページあたり20件のページネーション付き。
ステータス遷移ルール: 保留中→確認済み→発送済み→配送完了の順で進行し、保留中・確認済みからはキャンセルも可能。配送完了・キャンセルからの遷移は不可。
購入者は管理者の注文管理ページにアクセスできない。

## アクセス制御とリダイレクト
未ログインでカート・チェックアウト・注文履歴・注文詳細にアクセスした場合、ログインページにリダイレクトし、ログイン後に元のページに復帰する。購入者が管理者ページにアクセスした場合はアクセス拒否とする。
...
機能固有の設計指示のみ記述。TDD、スタブ置換、contracts 準拠、navLinks 有効化などの共通指示は constitution に定義済みのため省略可能。
